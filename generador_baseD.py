...
# RESERVAS (todos los alojamientos deben ser usados al menos una vez)
...
def crear_reserva_unica(cedula, id_aloj):
    global reserva_id
    intentos = 0
    while intentos < 10:
...
# 1. Garantizar una reserva por cada alojamiento
for id_aloj in alojamientos_ids:
    cedula = random.choice(huesped_cedulas)
    crear_reserva_unica(cedula, id_aloj)

# 2. (Opcional) Añadir algunas reservas adicionales
for _ in range(10):
    cedula = random.choice(huesped_cedulas)
